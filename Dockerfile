...
COPY . /app/.
RUN --mount=type=cache,id=TgWII6nnXw-/root/npm,target=/root/.npm npm ci
...
RUN --mount=type=cache,id=TgWII6nnXw-next/cache,target=/app/.next/cache --mount=type=cache,id=TgWII6nnXw-node_modules/cache,target=/app/node_modules/.cache npm run build
